...
class PnfOnboardStep(BaseStep):
    ...
    def execute(self) -> None:
        ...
        try:
            pnf: Pnf = Pnf.get_by_name(name=settings.PNF_NAME, vendor=vendor)
            if pnf.lifecycle_state == LifecycleState.CERTIFIED:
                return
        except ResourceNotFound:
            vsp: Vsp = Vsp(name=settings.VSP_NAME)
            pnf = Pnf.create(settings.PNF_NAME, vsp=vsp)
        ...
        if "pnfs" in self.yaml_template:
            ...
            for pnf in self.yaml_template["pnfs"]:
                ...
                try:
                    ...
                    pnf_obj: Pnf = Pnf.create(name=pnf["pnf_name"], vsp=vsp, vendor=vendor)
