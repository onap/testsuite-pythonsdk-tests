...
def get_parser():
    ...
    parser_bootstrap.add_argument(
        '--count', help='Instance count to bootstrap', type=int, metavar='INT', default=1)
    parser_bootstrap.add_argument('--urlves',
            help='URL of the VES collector', type=validate_url,
...
class MassPnfSim:
    ...
    def __init__(self):
        ...
        self.mvn_build_cmd = 'mvn clean package docker:build -Dcheckstyle.skip -X'
    ...
    def bootstrap(self, args): # pylint: disable=W0613
        ...
        Creates simulator folders with required settings and configs to run containers after.
        Values are adjusted based on initial settings provided in args.
    ...
    def build(self, args): # pylint: disable=W0613
        ...
        The simulator is written in Java and Maven is used to build docker images.
        ...
        for i in range(*self._get_iter_range()):
            ...
            try:
                ...
                with open(f'{self.sim_dirname_pattern}{i}/{self.sim_msg_config}') as data:
                    ...
                    sim_response = post(url, headers=req_headers, json=json_data)
                    if sim_response.status_code == codes.ok:
